...
const newDIDDocument = (didElement) => {
  ...
  core.addReadOnlyPropertyToObject(const { connected } = require("process");
  const { constants } = require("../constants");
  const { core } = require("../core");
  ...
  const newDIDDocument = (didElement) => {
      ...
      core.addReadOnlyPropertyToObject(
          document,
          "publicKey",
          getPublicKeyProperty(didElement)
      );
      core.addReadOnlyPropertyToObject(document, "authentication", [
      ...
      core.addReadOnlyPropertyToObject(
          document,
          "expires",
          getExpiration().toISOString().split(".")[0] + "Z"
      );
  ...
  const createVerifiableCredential = (
      ...
      core.addReadOnlyPropertyToObject(
          vc,
          "expirationDate",
          getExpiration(issuanceDate).toISOString().split(".")[0] + "Z"
      );
  ...
  const createService = (did, type, endpoint) => {
      let service = {};
  ...
  const createVerifiablePresentation = (
      ...
      core.addReadOnlyPropertyToObject(
          vp,
          "created",
          createDate.toISOString().split(".")[0] + "Z"
      );
      core.addReadOnlyPropertyToObject(vp, "verifiableCredential", [
          verifiableCredential,
      ]);
  ...
  const sealDocument = (didElement, document) => {
      ...
      let newDocument = {}
      core.setToJSON(newDocument);
      core.addReadOnlyPropertyToObject(newDocument, "id", document["id"])
      core.addReadOnlyPropertyToObject(newDocument, "publicKey", document["publicKey"])
      core.addReadOnlyPropertyToObject(newDocument, "authentication", document["authentication"])
  
      if (document.verifiableCredential && document.verifiableCredential.length > 0) {
          core.addReadOnlyPropertyToObject(newDocument, "verifiableCredential", document["verifiableCredential"])
      }
  
      if (document.service && document.service.length > 0) {
          core.addReadOnlyPropertyToObject(newDocument, "service", document["service"])
      }
  
      core.addReadOnlyPropertyToObject(newDocument, "expires", document["expires"])
      ...
      let dataToSign = Buffer.from(JSON.stringify(newDocument, null, ""), "utf8")
          .toString("hex")
          .toUpperCase();
      ...
      core.addReadOnlyPropertyToObject(newDocument, "proof", proof);
  ...
  const sign = (didElement, document) => {
      ...
      let dataToSign = Buffer.from(JSON.stringify(document, null, ""), "utf8")
          .toString("hex")
          .toUpperCase();
      let signature = core.signData(dataToSign, didElement.privateKey);
  ...
  const signVp = (didElement, document, realm, nonce) => {
      ...
      let dataToSign = Buffer.from(json + realm + nonce, "utf8")
          .toString("hex")
          .toUpperCase();
      let signature = core.signData(dataToSign, didElement.privateKey);
  ...
  const isValid = (diddocument, didElement, propertyName = "signatureValue") => {
      ...
      let dataToValidate = Buffer.from(JSON.stringify(document, null, ""), "utf8")
          .toString("hex")
          .toUpperCase();
  
      return core.verifyData(
          dataToValidate,
          diddocument["proof"][propertyName],
  ...
  const getMostRecentDIDDocument = async (did, options = {}) => {
    ...
    let useCache = true;
      ...
      if (options && "elastosRPCHost" in options)
          elastosRPCHost = options["elastosRPCHost"];
...
const sealDocument = (didElement, document) => {
  ...
  if (
    document.verifiablePresentation &&
    document.verifiablePresentation.length === 0
  )
    delete document.verifiablePresentation;
  if (document.service && document.service.length === 0)
